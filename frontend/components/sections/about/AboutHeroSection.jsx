    ...
    return (
        ...
        >
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                {/* Heading */}
                <motion.div
                    className={`text-center max-w-4xl mx-auto mb-20 ${motionGpu}`}
                    variants={staggerAlt}
                    initial="hidden"
                    animate="visible"
                >
                    <motion.div variants={fadeUpY40}>
                    ...
                    <motion.p variants={fadeUpY40} className="text-lg sm:text-xl text-slate-300 leading-relaxed">
                        {t.description}
                    </motion.p>
                </motion.div>
                ...
                >
                    {t.stats.map((stat, i) => (
                        <motion.div
                            key={i}
                            variants={scaleUpHero}
                            className={`text-center p-6 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm ${motionGpu}`}
                        >
                ...
                >
                    {visionMission.map(({ icon: Icon, title, text, gradient, iconBg, iconColor }) => (
                        <motion.div
                            ...
                            className={`relative p-8 rounded-3xl bg-white/5 border border-white/10 backdrop-blur-md overflow-hidden group ${motionGpu}`}
                        >
